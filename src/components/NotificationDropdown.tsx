...
const NotificationDropdown = () => {
  ...
  return <div className="relative" ref={dropdownRef}>
      <button className="relative p-2 rounded-lg hover:bg-[#f5f0e8] transition-colors" onClick={() => setIsOpen(!isOpen)} aria-label="Notifications">
        <BellIcon className="h-5 w-5 text-[#7a7067]" />
        {unreadCount > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-[#d4a5a5] text-white text-xs rounded-full flex items-center justify-center">
            {unreadCount}
          </span>}
      </button>
      {isOpen && <div className="fixed md:absolute left-4 right-4 md:left-auto md:right-0 top-16 md:top-full md:mt-2 bg-white rounded-xl shadow-lg z-[60] max-h-[calc(100vh-5rem)] md:max-h-[600px] md:w-96 flex flex-col">
          ...
          <div className="overflow-y-auto flex-grow">
            {notifications.length === 0 ? <div className="p-6 text-center text-[#7a7067] text-sm">
                No notifications
              </div> : <div className="divide-y divide-[#f5f0e8]">
                {notifications.map(notification => <div key={notification.id} className={`p-4 hover:bg-[#f5f0e8]/30 transition-colors ${!notification.read ? 'bg-[#f5f0e8]/10' : ''}`}>
