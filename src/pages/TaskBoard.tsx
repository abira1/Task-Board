...
const mockTasks: Task[] = [{
  ...
  description: 'Revise color palette and typography',
  status: 'todo',
  priority: 'Medium',
  assignee: {
    name: 'Emma Chen',
  ...
  description: 'Compile findings from recent interviews',
  status: 'inProgress',
  priority: 'High',
  assignee: {
    name: 'Jordan Lee',
      ...
      return prevTasks.map(t => {
        if (t.id === taskId) {
          const updatedTask = {
            ...
            status: boundedProgress === 100 ? 'done' : boundedProgress > 0 ? 'inProgress' : 'todo'
          };
          // Only notify on significant milestones
          if (boundedProgress % 25 === 0 || boundedProgress === 100) {
            addNotification({
              title: 'Progress Updated',
              message: `${t.title} is now ${boundedProgress}% complete`,
              type: 'task'
            });
          }
          return updatedTask;
